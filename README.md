...
**Всё «из коробки»: одна команда поднимает стек автоматически.**

```bash
make deploy        # создаст .env с секретами (если нет) и запустит всё
```

Первый запуск: `make deploy` генерирует `.env` из `.env.example` со случайными
секретами (если его нет), ставит запасной self-signed HTTPS-сертификат и
запускает PostgreSQL, бэкенд (миграции, seed, сборка сайта) и nginx. Сайт будет
доступен сразу по `https://<DOMAIN>` (с предупреждением браузера о
недоверенном сертификате).

Единственное, что стоит сделать руками на боевом сервере:

```bash
sed -i 's/cms.example.com/ВАШ_ДОМЕН/' .env     # или откройте .env в редакторе
make deploy
make ssl-init                                  # настоящий сертификат Let's Encrypt
```

Логин в админку `https://ВАШ_ДОМЕН/admin`: `DS_ADMIN_USERNAME` /
`DS_ADMIN_PASSWORD` из `.env` (по умолчанию `admin` + сгенерированный пароль).

### Что «из коробки» не может сломаться

- **`.env` не обязателен и безопасен** — генерируется автоматически; любое
  значение или его отсутствие не ломает запуск (у всего есть рабочие дефолты).
- **Несоответствие `POSTGRES_USER` / `DB_USER`** (и даже каракули в `.env`) —
  контейнер сам создаёт роль БД, базу и гранты при каждом старте
  (`docker/db_ensure.py`).
- **Опечатка в `DB_HOST`** — внутри сети compose адрес БД принудительно
  `db:5432`, переопределить и сломать это из `.env` нельзя.
- **Краш-луп** — если первый миграционный запуск упал, entrypoint не
  циклится с перезапусками, а поднимает сервер и повторяет попытку.

### Разработка на этой машине

```bash
make dev           # dev-стек: reload, порты 8080/8000/4321/5432
```
...
```bash
make deploy         # bootstrap-сертификат + подъём стека (всё автоматически)
